...
export class ChatService {
	...
	async findUser1(userid: number, chanid: number) {
		try {
			...
			if (user.user[0]) return user.user[0];
		} catch (error) {
			console.log(error);
		}
	}
	...
	async createMessage(createMessageDto: CreateMessageDto) {
		try {
			const muted = await this.findMuted(
	...
	async leaveRoom(client: Socket, oldChatId: number) {
		try {
			...
			if (oldChan) client.leave(oldChan.channelName);
		} catch (error) {
			console.log(error);
		}
	}
	...
	async leaveChannel(chan: any, userid: number) {
		try {
			...
			if (userInChan) return userInChan;
		} catch (error) {
			console.log(error);
		}
	}
	...
	async updatePassword(pass: string, chanid: number) {
		try {
			...
			if (updateChan) return this.exclude(updateChan, ['password']);
		} catch (error) {
